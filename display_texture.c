...
unsigned int	get_texture_pixel(t_game *game, int idx_x, int cur_idx_y)
{
	t_ray * const		cur_ray = &game->ray_data[idx_x];
	t_vector2 * const	wall_range = &game->wall_range[idx_x];
	t_img_data			*texture_img;
	t_vector2			ratio_pos_texture;
	t_vector2_d			texture_idx;

	texture_img = &game->texture[cur_ray->hit_wall_side].img_data;
	texture_idx.x = game->texture[cur_ray->hit_wall_side].texture_width;
	texture_idx.y = game->texture[cur_ray->hit_wall_side].texture_height;
	ratio_pos_texture.x = cur_ray->hit_texture_point;
	ratio_pos_texture.y = \
				(cur_idx_y - wall_range->x) / (wall_range->y - wall_range->x);
	texture_idx.x *= ratio_pos_texture.x;
	texture_idx.y *= ratio_pos_texture.y;
	return (*(unsigned int *)(texture_img->addr + \
			texture_idx.y * texture_img->line_length + texture_idx.x * \
...
void	put_pixel_wall(t_game *game, int idx_x)
{
	...
	while (cur_idx_y < game->wall_pixel[idx_x].y)
	{
		...
		color = get_texture_pixel(game, idx_x, cur_idx_y);
