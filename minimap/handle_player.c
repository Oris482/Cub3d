...
void	check_wall_collisions(t_game *game, unsigned int const pressed_keyset)
{
...
void    move_player(t_player *player, char **map, unsigned int const pressed_keyset)
{
	...
	player_pos = &player->vec_pos;
	if (!!(pressed_keyset & KEYSET_W) ^ !!(pressed_keyset & KEYSET_S))
	{
		sign = 1.0;
		if (pressed_keyset & KEYSET_S)
			sign = -1.0;
		// player_pos->x += move_speed * cut_point(cos(deg2rad(angle)), 6) * sign;
		// player_pos->y += move_speed * cut_point(sin(deg2rad(angle)), 6) * sign;
		tmp_pos.x = player_pos->x + move_speed * cut_point(cos(deg2rad(angle)), 6) * sign;
		tmp_pos.y = player_pos->y + move_speed * cut_point(sin(deg2rad(angle)), 6) * sign;
		if (map[(int)ceil(tmp_pos.y)][(int)ceil(tmp_pos.x)] != WALL)
		{
	...
	if (!!(pressed_keyset & KEYSET_A) ^ !!(pressed_keyset & KEYSET_D))
	{
		...
		tmp_pos.x = player_pos->x + move_speed \
				* cut_point(cos(deg2rad(adjust_degree(angle, 90.0 * sign))), 6);
		tmp_pos.y = player_pos->y + move_speed \
				* cut_point(sin(deg2rad(adjust_degree(angle, 90.0 * sign))), 6);
		if (map[(int)ceil(tmp_pos.y)][(int)ceil(tmp_pos.x)] != WALL)
		{
			player_pos->x = tmp_pos.x;
